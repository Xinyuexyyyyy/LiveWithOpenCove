...
def _load_harvest_dependencies() -> tuple[
    ...
    """Locate the sibling harvest-tool repo and import its discover/harvest functions.

    Search order:
      1. $HARVEST_TOOL_ROOT (explicit override)
      2. <workspace>/skills/harvest-tool (legacy in-tree layout)
      3. ~/harvest-tool, ~/code/harvest-tool, ~/projects/harvest-tool (common clones)

    The `github-build` route requires harvest-tool (https://github.com/Xinyuexyyyyy/harvest-tool).
    Other routes (product, social, needs-clarification) do not need it.
    """
    candidates: list[Path] = []
    env_root = os.environ.get("HARVEST_TOOL_ROOT")
    if env_root:
        candidates.append(Path(env_root).expanduser())
    candidates.append(_workspace_root() / "skills" / "harvest-tool")
    home = Path.home()
    candidates.extend([home / "harvest-tool", home / "code" / "harvest-tool", home / "projects" / "harvest-tool"])

    harvest_root = next((p for p in candidates if (p / "scripts" / "discover.py").is_file()), None)
    if harvest_root is None:
        raise RuntimeError(
            "idea-to-research's github-build route requires harvest-tool. "
            "Install it from https://github.com/Xinyuexyyyyy/harvest-tool and either:\n"
            "  - clone to one of: ~/harvest-tool, ~/code/harvest-tool, ~/projects/harvest-tool, or\n"
            "  - set HARVEST_TOOL_ROOT to the clone path."
        )
